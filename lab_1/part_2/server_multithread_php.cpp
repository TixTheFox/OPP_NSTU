...
void* acceptQuery(void* arg) {
    ...
    FILE *fp;
    fp = popen("php -r 'echo phpversion();'", "r");
    if (fp == NULL) {
        strcpy(buffer, "Error executing PHP script");
    } else {
        fgets(buffer, sizeof(buffer), fp);
        pclose(fp);
    }

    char new_response[1024];
    snprintf(new_response, sizeof(new_response), 
        "HTTP/1.1 200 OK\r\n"
    ...
    send(*client_fd, new_response, sizeof(new_response), 0);
    close(*client_fd);
    delete client_fd;
}
