...
int main(int argc, char *argv[]) {
    ...
    // переменные для хранения информации о времени
    time_point t1, t2;
    double all_time = 0;    // полезное время вычислений процесса
    double comm_time = 0;   // время коммуникаций
    double dop_time = 0;    // дополнительные операции (копирование данных при приеме и отправке)
    ...
    if (rank == 0) {
        ...
        do {
            ...
            all_time += std::chrono::duration<double>(t2-t1).count();
            ...
            t2 = std::chrono::high_resolution_clock::now();
            dop_time += std::chrono::duration<double>(t2-t1).count();
            ...
            t2 = std::chrono::high_resolution_clock::now();
            comm_time += std::chrono::duration<double>(t2-t1).count();
    ...
    // Вывод времени с блокировкой
    for (int p = 0; p < size; p++) {
        if (p == rank) {
            printf("\nПроцесс %d:\n", rank);
            printf("Общее время вычислений: %.4f секунд\n", all_time);
            printf("Время коммуникаций: %.4f секунд\n", comm_time); 
            printf("Дополнительное время: %.4f секунд\n", dop_time);
            fflush(stdout);
        }
        MPI_Barrier(MPI_COMM_WORLD);
    }
