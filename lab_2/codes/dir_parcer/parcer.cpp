...
/*
    Функция для нахождения подстроки в файле filename
    Возвращает true при обнаружении первого вхождения, иначе false
*/
...
void* working_thread(void*) {
    ...
    while(true) {
        ...
        if (task.empty()) {
            ...
            // иначе если файл - не директория, то пытаемся найти строку в нем
            bool res = fileFindSubstring(task);
            if (res) {
                // безопасно выводим имя файла, если он содержит искомую подстроку
                pthread_spin_lock(&console_out_spin);
                cout << task << "\n";
                pthread_spin_unlock(&console_out_spin);
            }
...
int main(int argc, char* argv[]) {
    ...
    // устанавливаем начальную задачу - введенная пользователем директория
    tasks.push(argv[1]);
    // так же устанавливаем искомую строку
    search_str = argv[2];
